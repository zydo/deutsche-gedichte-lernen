...
> 12 首在扣除三级来源后独立来源不足。**截至 2026 年 8 月已全部补验证完毕；当前 58 首中 57 首通过、仅剩 1 首 WARN**；
...
- 每首诗的德文原文必须有明确、可核实的出处。01–33 的既有语料至少用**两个相互独立的来源**交叉核对；编号 ≥34 的新增诗须至少 3 个独立运营方，且至少 1 个一级来源。**"什么算独立来源"由 [SOURCES.md](SOURCES.md) 强制规定**（含运营方分组表与来源分级），并由 `npm run check` 机器校验——这条规则不是口头约定，是可执行的。
...
**关于配图**：前 50 首（编号 01–50）均已配有 AI 生成插图（`public/images/`，4:3 横版）；第六批 8 首（编号 55–62）仍以 image brief 占位。若某首诗的 `image_path` 为 `null`，页面会自动渲染完整 brief（意象、情绪、画面元素、时代感、推荐风格、禁忌内容）；详见[如何添加或替换配图](#如何添加或替换配图)。
...
│       ├── ...                  # 58 个已发布文件（01–50、55–62）
│       └── pending/             # 4 个未到公版日的条目（51–54，正文留空）
├── public/
│   ├── style.css                # 主样式表（纸张质感 / 衬线字体 / 配色变量）
│   └── images/                  # 配图（50 张，对应编号 01–50）
├── src/
...
唯一的例外是**开发期的代码格式化**：`prettier` 作为 `devDependencies` 仅供 `npm run format` 使用，不参与构建、不影响部署。如果你要参与提交，请执行：

```bash
npm install # 只安装 prettier
```

不安装也能正常构建和部署；需要格式化时运行 `npm run format`。
...
npm run format   # 格式化仓库代码与文档（需先 npm install）
```
...
2. 对每首诗做字段、计数、跨页引用、韵式、公版年份与配图状态校验；内容错误会中断构建，模板缺失字段也会在终端显式预警；
...
5. 复制样式表与配图到 `dist/`，并从 `dist/poems`、`dist/snapshots`、`dist/images` 清理已无来源的旧生成文件。

构建产物 `dist/` 是完全独立的静态文件集合，可以直接拖拽到 Netlify / Vercel / GitHub Pages / 任意静态文件服务器上部署，无需服务端运行环境。

`build.js` 不会清空整个 `dist/`，只同步由构建脚本全权管理的三个子目录；因此重命名 slug、删除诗歌或撤下图片后不会遗留旧页面或旧资源，也不会误删 `dist/` 根目录的其他文件。

---

## 如何添加一首新诗

**不需要写任何新代码**，只需要在 `data/poems/` 目录下新建一个 JSON 文件（文件名建议格式：`两位数字前缀-拼音或英文slug.json`，例如 `63-an-die-freude.json`），并填写以下字段（可参照任意一个现有文件作为模板，例如 `data/poems/01-heidenroeslein.json`）：
...
| `german_sources`        | `{name, url, tier, citation?, note?}[]`                                                           | 新增诗至少 3 个独立运营方，且至少 1 个一级来源                                                  |
...
1. 从至少 3 个独立运营方核对原文，其中至少 1 个一级来源并记录纸本版次/页码（完整分级与独立性规则见 [SOURCES.md](SOURCES.md)）；
2. 记录版本/拼写/标点异文；
...
- **德文原文出处**（`german_sources`）：每条记录包含来源名称、完整 URL、来源级别，以及该来源在校对中起到的作用。01–33 的遗留最低线为 2 个独立运营方；编号 ≥34 的新增诗须至少 3 个独立运营方，且至少 1 个一级来源并填写 `citation`。判定标准、运营方分组表与来源分级见 **[SOURCES.md](SOURCES.md)**，务必先读。同一网站的镜像/转载、以及同一运营方的不同域名（典型例子：`gedichte7.de` 与 `zgedichte.de`），**都不算两个独立来源**。填好后运行 `npm run check` 自查。
...
- **德文原文**：目前收录的 58 首诗均已进入公有领域。逝世最晚的是 Else Lasker-Schüler（1945 年，其作品自 2016 年起进入公有领域），其次为 Stramm（1915）、Stadler 与 Morgenstern（均 1914）、Heym（1912）。编号 ≥34 的录入在 JSON 的 `public_domain` 字段中保存公版年份复算记录，并由 `npm run check` 机器校验。
...
- **★ 配图优先于扩容**：一首有图的诗对学习者的价值，高于一首没图的诗。目前仍有第六批 8 首待配图；本站之后**语料扩容应让位于既有条目的深化**（配图、待核项落地、人工复核），直到这 8 首全部完成为止。
